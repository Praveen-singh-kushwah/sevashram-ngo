...
const ProgramsSection = () => {
  ...
  useEffect(() => {
    const loadPrograms = async () => {
      try {
        const res = await fetchHomePrograms({ limit: 4 });
        console.log('Fetched home programs:', res.data);
        setPrograms(res.data || []);
      } catch (err) {
        console.error('Failed to load home programs', err);
      } finally {
        setLoading(false);
      }
    };
...
const ProgramCard = ({ program, variants }) => {
  const { title, slug, shortDescription, cover } = program;
